...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Aplica a U-Net multiclasse intrarrenal dentro das mascaras renais "
            "existentes em dataset_geral."
        )
    )
...
def predict_roi(bundle, roi_image, roi_kidney, device):
    ...
    model_image = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8)).apply(resized)
    image_float = model_image.astype(np.float32) / 255.0
    ...
    with torch.no_grad():
        prediction = bundle["model"](tensor).argmax(dim=1).cpu().numpy()[0].astype(np.uint8)
...
def generate_one(bundle, row, args, device):
    ...
    roi_prediction = predict_roi(bundle, roi_image, roi_kidney, device)
...
def main():
    ...
    summary = {
        ...
        "architecture": "unet_multiclass_roi",
