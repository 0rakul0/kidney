...
\title{Um \textit{Pipeline} de Vis\~ao Computacional para An\'alise de Achados Hiperecog\^enicos em tUltrassonografia Renal}
...
\begin{abstract}
Renal ultrasound interpretation depends on subjective visual inspection and anatomical delimitation of the kidney. This paper proposes a computer vision \textit{pipeline} that first segments the kidney and then analyzes suspicious hyperechoic regions near renal pyramids. An exploratory annotated set was used to train an initial model, which generated pseudo masks accepted only above a 90\% operational confidence threshold. After data expansion, a second DeepLabV3 ResNet50 model was trained on the consolidated set and achieved Dice of 0.9235, IoU of 0.8741 and precision of 0.9301 on a fixed test set. The fibrosis suspicion classifier remains exploratory due to the lack of real labeled examples.
\end{abstract}

\begin{resumo}
A interpreta\c{c}\~ao de ultrassonografias renais depende de inspe\c{c}\~ao visual subjetiva e da delimita\c{c}\~ao anat\^omica do rim. Este artigo prop\~oe um \textit{pipeline} de vis\~ao computacional que primeiro segmenta o rim e depois analisa regi\~oes hiperecog\^enicas suspeitas pr\'oximas \`as pir\^amides renais. Uma base explorat\'oria anotada foi usada para treinar um modelo inicial, que gerou pseudom\'ascaras aceitas apenas acima de 90\% de confian\c{c}a operacional. Ap\'os a expans\~ao dos dados, um segundo DeepLabV3 ResNet50 foi treinado no conjunto consolidado e atingiu Dice de 0,9235, IoU de 0,8741 e precis\~ao de 0,9301 no teste fixo. O classificador de suspeita de fibrose permanece explorat\'orio pela falta de exemplos reais rotulados.
\end{resumo}

\section{Introdu\c{c}\~ao}

A ultrassonografia renal \'e acess\'ivel, n\~ao invasiva e amplamente usada na avalia\c{c}\~ao de altera\c{c}\~oes estruturais do rim. Na pr\'atica cl\'inica, regi\~oes muito claras no par\^enquima, especialmente pr\'oximas \`as pir\^amides renais, podem indicar achados suspeitos que exigem an\'alise cuidadosa.
...
quantitativas de intensidade e textura para investigar padrões associados a alterações
renais~\cite{Chavhan2008}. Ainda assim, permanece a lacuna entre segmentar o rim e examinar, dentro dele, pontos hiperecog\^enicos candidatos a achados suspeitos.
...
para investigação de alterações renais\cite{Chavhan2008}.

As pir\^amides renais s\~ao estruturas da medula do rim, entre o c\'ortex e o sistema coletor, que servem como refer\^encias anat\^omicas internas em ultrassonografia. Altera\c{c}\~oes de ecogenicidade nessas pir\^amides foram associadas a pior fun\c{c}\~ao renal em cen\'arios cl\'inicos espec\'ificos~\cite{Chavhan2008}, o que motiva a busca por estruturas claras nessa regi\~ao.
...
No pr\'oprio SBBD, trabalhos recentes discutem organiza\c{c}\~ao de dados de sa\'ude para aprendizagem computacional~\cite{Lima2021DataLakeSaude} e aprendizado ativo para reduzir rotula\c{c}\~ao~\cite{Constantino2022SBBD}. Este trabalho aproxima essas linhas em um \textit{pipeline} de dados no qual a m\'ascara segmentada delimita a busca por regi\~oes claras pr\'oximas \`as pir\^amides candidatas.

\section{Metodologia}

A metodologia foi organizada para transformar uma imagem de ultrassonografia renal em uma regi\~ao anat\^omica delimitada e, em seguida, extrair medidas de achados hiperecog\^enicos pr\'oximos \`as pir\^amides renais.

Para isso, o \textit{pipeline} primeiro prepara a entrada, segmenta o rim, restringe a an\'alise \`a m\'ascara renal, procura regi\~oes claras intrarrenais e, por fim, produz atributos que podem alimentar uma etapa explorat\'oria de suspeita de fibrose. A Figura~\ref{fig:tikz_pipeline} apresenta essa organiza\c{c}\~ao geral.
...
\node[block, right=of feat] (clf) {Suspeita\\explorat\'oria};
\draw[->, thick] (input) -- (prep);
\draw[->, thick] (prep) -- (seg);
\draw[->, thick] (seg) -- (feat);
\draw[->, thick] (feat) -- (clf);
\end{tikzpicture}
\caption{\textit{Pipeline} metodol\'ogico: a segmenta\c{c}\~ao renal isola o rim para an\'alise de regi\~oes hiperecog\^enicas pr\'oximas \`as pir\^amides renais.}
...
Quando h\'a ROI manual do f\'igado, tamb\'em s\~ao calculadas raz\~oes entre rim e f\'igado. A etapa final usa um Random Forest~\cite{Breiman2001} apenas como prova de conceito, pois os achados claros ainda s\~ao medidas explorat\'orias e n\~ao uma decis\~ao robusta de fibrose em sim ou n\~ao.
...
A classifica\c{c}\~ao de suspeita de fibrose usa apenas 50 imagens rotuladas, com 44 para treino e valida\c{c}\~ao e 6 para teste. Por esse motivo, essa etapa permanece explorat\'oria e ainda n\~ao fecha diagn\'ostico de fibrose em sim ou n\~ao.

Essa separa\c{c}\~ao entre segmenta\c{c}\~ao e decis\~ao cl\'inica foi adotada deliberadamente. O primeiro problema exige aprender o contorno renal com boa estabilidade; o segundo exige exemplos reais de fibrose, preferencialmente balanceados e associados a informa\c{c}\~oes cl\'inicas. Neste est\'agio, portanto, o artigo prioriza a etapa que j\'a pode ser avaliada de forma consistente.
...
Na Figura~\ref{fig:model2_proof}, a sa\'ida do segundo segmentador n\~ao \'e tratada como diagn\'ostico final. Ela define a janela anat\^omica para extrair atributos de intensidade, textura e rela\c{c}\~ao espacial. Assim, a etapa de suspeita de fibrose permanece explorat\'oria: verifica se os atributos extra\'idos da regi\~ao renal carregam algum sinal, mas ainda depende de mais exemplos reais e balanceados para sustentar uma decis\~ao cl\'inica em sim ou n\~ao.
...
Além do desempenho quantitativo, o pipeline preserva a separação entre evidência computacional e interpretação clínica. Nesse contexto, as regiões hiperecogênicas são tratadas como achados candidatos para investigação, e não como diagnóstico automático. Ainda assim, a etapa de suspeita de fibrose permanece exploratória devido à limitação de exemplos clínicos rotulados e ao reduzido conjunto balanceado de casos confirmados.
